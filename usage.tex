...
\vignette{Twissistency's data scientists have learned that their users
  respond negatively to slow updates, and the company sets a
  $t$-visibility target of 500ms at the 99.9th percentile for their
...
\vignette{Based on feedback from their infrastructure team, the
  ...
  the consistency SLA is violated.}
...
\vignette{The consistency autotuner uses R=W=1 for most traffic but
  ...
  also viable, the autotuner determines that the 99.9th percentile
  write latency is much higher than the read latency, which is served
  mostly from cache.}
...
\vignette{The administrators inspect the consistency metrics for each
  ...
  spin up a new replica set on a different rack.}
